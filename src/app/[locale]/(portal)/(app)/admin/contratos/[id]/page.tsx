...
export default async function ContractDetailPage({
  ...
  const equipmentIds = contract.equipmentLinks.map((l) => l.equipmentId);
  ...
  const totalModules = contract.equipmentLinks.reduce(
    (a, l) => a + l.equipment.modules.length,
    0,
  );
    ...
    <div className="mx-auto max-w-4xl space-y-6">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {admin && (
            <Button asChild variant="outline" size="sm">
              <Link href={`/admin/users/${contract.client.id}/equipos`}>
                Equipos del lab
              </Link>
            </Button>
          )}
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        ...
        <Card className="p-5">
          ...
          <div className="mt-1.5 text-2xl font-semibold">
            {contract.equipmentLinks.length}
      ...
      <Card>
        ...
        {contract.equipmentLinks.length === 0 ? (
          <p className="px-5 py-10 text-center text-sm text-muted-foreground">
            Este contrato no ampara equipos.
          </p>
        ) : (
          <div className="divide-y divide-border">
            {contract.equipmentLinks.map(({ equipment: eq }) => {
