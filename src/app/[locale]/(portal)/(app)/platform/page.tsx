...
import { Building2, ShieldAlert, Database, LogIn, History } from "lucide-react";
import { auth } from "@/lib/auth";
import { redirect } from "@/i18n/navigation";
import { getTenants, getPlatformEvents } from "@/lib/data/platform";
...
export default async function PlatformPage({
  ...
  // Un administrador de empresa NO entra aquí: es otra dimensión de permiso.
  if (!session?.user?.platformRole) {
    redirect({ href: "/dashboard", locale });
  }
  const isSuper = session!.user.platformRole === "superadmin";

  const [rows, events, active] = await Promise.all([
    getTenants(),
    ...
    <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <Badge className="bg-primary/15 text-primary ring-primary/25">
              {isSuper ? "Superadministrador" : "Soporte"}
            </Badge>
          </div>
      ...
      {active?.impersonated && (
        <Card className="border-warning/40 bg-warning/5 p-4">
          <p className="text-sm">
            Estás dentro de{" "}
            <span className="font-semibold">{active.name}</span> como personal de
            plataforma. Este acceso quedó registrado.
          </p>
        </Card>
      )}
